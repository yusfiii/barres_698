...
// 1. Data Summary Total (Satu Tahun)
$query_total = "
    SELECT 
        SUM(jumlah_bangunan) as tot_bangunan,
        SUM(jumlah_KK) as tot_kk,
        SUM(jumlah_individu) as tot_individu,
        SUM(korban_luka) as tot_luka,
        SUM(korban_jiwa) as tot_jiwa
...
$tot_bangunan = $res_total['tot_bangunan'] ?? 0;
$tot_kk = $res_total['tot_kk'] ?? 0;
$tot_individu = $res_total['tot_individu'] ?? 0;
$tot_luka = $res_total['tot_luka'] ?? 0;
$tot_jiwa = $res_total['tot_jiwa'] ?? 0;

// 2. Data Tren Bulanan untuk Dampak/Korban
$query_bulanan = "
    SELECT MONTH(waktu) as bulan, 
           SUM(jumlah_bangunan) as sum_bangunan,
           SUM(jumlah_KK) as sum_kk,
           SUM(jumlah_individu) as sum_individu,
           SUM(korban_luka) as sum_luka,
           SUM(korban_jiwa) as sum_jiwa
    FROM kejadian_kebakaran 
    WHERE YEAR(waktu) = ?
...
$data_bulanan = array_fill(1, 12, [
    'bangunan' => 0, 'kk' => 0, 'individu' => 0, 'luka' => 0, 'jiwa' => 0
]);

while ($row = $res_bulanan->fetch_assoc()) {
    $data_bulanan[$row['bulan']] = [
        'bangunan' => $row['sum_bangunan'] ?? 0,
        'kk' => $row['sum_kk'] ?? 0,
        'individu' => $row['sum_individu'] ?? 0,
        'luka' => $row['sum_luka'] ?? 0,
        'jiwa' => $row['sum_jiwa'] ?? 0
    ];
}
...
<head>
    ...
    <title>Laporan Statistik Korban - BARRES 698</title>
    ...
    <style>
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .laporan .kop-surat {
            ...
            border-bottom: 3px solid #000 !important; padding-bottom: 15px !important; margin-bottom: 20px !important; position: relative !important;
        }
        .laporan .kop-surat img { position: static !important; height: 75px !important; width: auto !important; margin-right: 20px !important; transform: none !important; }
        .laporan .kop-surat .kop-text { flex: 1 !important; text-align: center !important; padding-right: 95px !important; }
        .laporan .kop-surat .nama-organisasi { font-size: 16pt !important; font-weight: 800 !important; margin: 0 0 5px 0 !important; color: #000 !important; line-height: 1.2 !important; }
        .laporan .kop-surat p { font-size: 9.5pt !important; color: #333 !important; margin: 2px 0 !important; line-height: 1.4 !important; }
        ...
        .laporan .data-table th, .laporan .data-table td { border: 1px solid #666; padding: 6px 8px; font-size: 10pt; vertical-align: middle; }
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="top-navbar no-print">
            <div class="page-title">
                <h2>Laporan Korban & Dampak</h2>
                <p>Analisis statistik bangunan rusak dan korban (KK, Individu, Luka, Jiwa)</p>
            </div>
            ...
            <form method="GET" action="" class="row w-100 g-3 align-items-end m-0">
                ...
                <div class="col-md-3">
                    <button type="submit" class="btn-gold w-100">
                        <i class="fas fa-chart-pie"></i> Proses Data
                    </button>
                </div>
                <div class="col-md-4">
        ...
        <div class="preview-container laporan" id="laporanPreview">
            ...
            <div class="surat-info">
                <div class="left">
                    <span class="label">Nomor</span> : 028/BARRES698/<?= date('m/Y') ?><br>
                    <span class="label">Perihal</span> : Laporan Statistik Korban & Dampak Kebakaran Tahun <?= $filter_tahun ?>
                </div>
                <div class="right">
            ...
            <div class="judul">STATISTIK KORBAN & DAMPAK TAHUN <?= $filter_tahun ?></div>

            <p style="text-align: justify; margin-bottom: 15px;">
                Laporan ini menyajikan akumulasi dampak kerusakan fisik dan korban jiwa/luka akibat insiden kebakaran di Kota Banjarbaru sepanjang tahun <strong><?= $filter_tahun ?></strong>. Secara keseluruhan tercatat: <strong><?= $tot_bangunan ?></strong> unit bangunan terdampak, mempengaruhi <strong><?= $tot_kk ?></strong> Kepala Keluarga (KK) yang terdiri dari <strong><?= $tot_individu ?></strong> jiwa. Tercatat <strong><?= $tot_luka ?></strong> korban luka dan <strong><?= $tot_jiwa ?></strong> korban jiwa.
            </p>
            ...
            <p style="text-align: justify; margin-bottom: 10px;">
                <strong>Rincian Perbandingan Dampak per Bulan:</strong>
            </p>

            <!-- Tabel Rekapitulasi Korban -->
            <table class="data-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 15%;">Total KK</th>
                        <th style="width: 15%;">Total Individu</th>
                        <th style="width: 15%;">Korban Luka</th>
                        <th style="width: 15%;">Korban Jiwa</th>
                    </tr>
                </thead>
                <tbody>
                    <?php for ($i = 1; $i <= 12; $i++): 
                    ...
                    ?>
                        <tr>
                            ...
                            <td><?= $nama_bulan[$i] ?></td>
                            <td style="text-align: center;"><?= $d['bangunan'] ?: '-' ?></td>
                            <td style="text-align: center;"><?= $d['kk'] ?: '-' ?></td>
                            <td style="text-align: center;"><?= $d['individu'] ?: '-' ?></td>
                            <td style="text-align: center; color: <?= $d['luka'] > 0 ? '#dc3545' : 'inherit' ?>; font-weight: <?= $d['luka'] > 0 ? 'bold' : 'normal' ?>;">
                                <?= $d['luka'] ?: '-' ?>
                            </td>
                            <td style="text-align: center; color: <?= $d['jiwa'] > 0 ? '#dc3545' : 'inherit' ?>; font-weight: <?= $d['jiwa'] > 0 ? 'bold' : 'normal' ?>;">
                                <?= $d['jiwa'] ?: '-' ?>
                            </td>
                        </tr>
                    <?php endfor; ?>
                ...
                <tfoot>
                    <tr style="background: #ECECEC; font-weight: bold;">
                        ...
                        <td style="text-align: center;"><?= $tot_bangunan ?></td>
                        <td style="text-align: center;"><?= $tot_kk ?></td>
                        <td style="text-align: center;"><?= $tot_individu ?></td>
                        <td style="text-align: center; color: #dc3545;"><?= $tot_luka ?></td>
                        <td style="text-align: center; color: #dc3545;"><?= $tot_jiwa ?></td>
                    </tr>
                </tfoot>
            </table>
    ...
    <script>
        ...
        // 1. Chart Data Total Dampak
        const ctxTotal = document.getElementById('totalDampakChart').getContext('2d');
        new Chart(ctxTotal, {
            type: 'bar',
            data: {
                labels: ['Bangunan', 'Kepala Keluarga (KK)', 'Total Individu', 'Korban Luka', 'Korban Jiwa'],
                datasets: [{
                    label: 'Jumlah Tercatat',
                    data: [<?= $tot_bangunan ?>, <?= $tot_kk ?>, <?= $tot_individu ?>, <?= $tot_luka ?>, <?= $tot_jiwa ?>],
                    backgroundColor: [
                        '#fd7e14', // Bangunan - Orange
                        '#17a2b8', // KK - Cyan
                        '#007bff', // Individu - Blue
                        '#ffc107', // Luka - Yellow
                        '#dc3545'  // Jiwa - Red
                    ],
                    borderColor: '#1A1A1A',
                    ...
                    tooltip: {
                        callbacks: {
                            label: function(context) {
                                return context.parsed.y + ' Kasus/Orang';
                            }
                        }
                    }
                },
